...
function App() {
  ...
  const fetchPokeData = async (isFirstFetch) => {
    try {
      ...
      const url = `https://pokeapi.co/api/v2/pokemon/?limit=${20}&offset=${offsetValue}`;
  ...
  return (
    <article className="pt-6">
        ...
        <div className="relative z-50">
          <form className="relative flex justify-center items-center w-[20.5rem] h-6 rounded-lg m-auto">
            <input
              ...
              value={searchTerm}
              className="text-xs w-[20.5rem] h-6 py-1 rounded-lg text-gray-300 text-center bg-[hsl(214,13%,47%)]"
            />
            <button
              type="submit"
      ...
      <div className="text-center">
        <button
          className="bg-sky-800 px-6 py-2 text-base rounded-lg font-bold text-white"
          onClick={() => {
            fetchPokeData(false);
          }}
        >
